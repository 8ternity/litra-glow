...
let BrightnessDownAction = (() => {
    ...
    (class extends _classSuper {
        ...
        async setLitraBrightness(lumens) {
            return new Promise((resolve, reject) => {
                logToFile(`[setLitraBrightness] Exécution directe: litra-brightness-lm ${lumens}`);
                // Utiliser spawn pour exécuter la commande exactement comme en ligne de commande
                const cmd = child_process.spawn('npx', ['litra-brightness-lm', `${lumens}`], {
                    cwd: process.cwd(),
                    env: {
                        ...process.env,
                        PATH: process.env.PATH + ';C:\\Program Files\\nodejs\\;C:\\Users\\ET34N1TY\\AppData\\Roaming\\npm',
                ...
                cmd.on('close', (code) => {
                    if (code === 0) {
                        logToFile(`[setLitraBrightness] Commande exécutée avec succès: ${stdout.trim()}`);
                        resolve();
                    }
                    else {
                        logToFile(`[setLitraBrightness] Erreur (code ${code}): ${stderr.trim()}`);
        ...
        async onKeyDown(ev) {
            ...
            try {
                ...
                // Application de la nouvelle luminosité avec la nouvelle méthode
                logToFile(`[onKeyDown] Application de la nouvelle luminosité: ${newLumens} lm`);
                await this.setLitraBrightness(newLumens);
                // Pause pour laisser le temps à la lampe de s'ajuster
...
//# sourceMappingURL=brightness-down-LKuREXnf.js.map
